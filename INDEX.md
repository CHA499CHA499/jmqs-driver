...
> 最后核对：2026-08-21。本文以当前工作树中的 `app/`、`scripts/`、`tests/`、`public/` 与 `package.json` 为真源；历史说明或旧部署与代码冲突时，以代码为准。
...
自动化基线（2026-08-21）：`npm test` 构建成功，102 项测试中 100 通过、2 跳过、0 失败。跳过项均来自真实本地端口绑定测试，不得据此声称 HTTP 运行态已验收。`npm run lint` 为 0 error、18 个既有 `<img>` 性能 warning。

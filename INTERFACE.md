...
- YouNavi 按 L0 `<导入目录>/SKILL.md` 将其识别为 `jmqs-driver`；根 Skill 中 `${SKILL_DIR}` 必须替换为该目录绝对路径。
- `.agents/skills/persona-driver-setup/` 是根 Skill 调用的内部安装实现，不是要求用户另行导入的第二个分发包。
- 项目被导入为 `<YouNavi Skills 根>/jmqs-driver` 且未设置 `PERSONA_NAVI_SKILLS_DIR` 时，安装脚本将项目父目录识别为 Skills 根；五个人物 Skill 与 `create-soul` 安装到该目录下的同级子目录。
- 五个人物 Skill 的安装源固定为随包 `.agents/skills/persona-driver-setup/assets/persona-skills/<skill>/`。setup 在复制前按 `persona-driver.bundled-persona-skills/v1` 校验 name、公开来源、commit、文件数、字节数和聚合 SHA-256；首次安装不得调用 `git`、GitHub 或代理。
